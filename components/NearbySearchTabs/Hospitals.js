...
setHospitals(hospitalData);
  ...
  const filteredHospital = hospitals.filter((hosp) => {
    distance = calculateDistance(13.0461852,80.0932303,hosp.location.lat,hosp.location.lon).toFixed(1);
  //  setDistance(dist);
   console.log("Hospital distance"+distance);
   return distance < 10;
 });
      ...
      <FlatList
        ...
        renderItem={({item})=> (
          <View style={tw`px-5 mt-1 mb-4 `}>
            <HospitalCard distance = {distance} {...item}/>
          </View>
        )}
      />
    </View>
